...
| **Teacher's reminders** (REQ-057) | Until the teacher deletes them | `Reminder.date` | `DELETE /api/reminders/{id}` — the teacher's own call; one naming a family is erased with that family's data on request |
