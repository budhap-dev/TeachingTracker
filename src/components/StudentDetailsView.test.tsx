...
describe('StudentDetailsView', () => {
  it('renders details and triggers back/edit/progress callbacks', async () => {
    ...
    expect(screen.getByText((_, element) => element?.textContent === 'Subjects: Mathematics, Physics')).toBeInTheDocument();

    await user.click(screen.getByRole('button', { name: /back to students/i }));
    expect(onBack).toHaveBeenCalledTimes(1);

    await user.click(screen.getByRole('button', { name: /^edit$/i }));
    expect(onBeginEdit).toHaveBeenCalledWith(expect.objectContaining({ id: 10 }));

    fireEvent.change(screen.getByRole('slider', { name: /progress/i }), { target: { value: '92' } });
  ...
  it('renders editing state with fallbacks and save/cancel callbacks', async () => {
    ...
    expect(screen.getByText((_, element) => element?.textContent === 'Date of birth: Not provided')).toBeInTheDocument();
