...
type PaymentTrackerViewProps = {
  ...
  onUpdatePaymentRecord: (record: { studentId: number; month: string; status: PaymentStatus; amountPaid: number; notes: string }) => void;
};
...
export const PaymentTrackerView = ({ students, paymentRecords, onUpdatePaymentRecord }: PaymentTrackerViewProps) => {
  const monthOptions = useMemo(getMonthOptions, []);
  const currentMonth = `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, '0')}`;
  const defaultMonth = currentMonth;
  const [selectedMonth, setSelectedMonth] = useState(defaultMonth);

  const monthRecords = useMemo(() => paymentRecords.filter((record) => record.month === selectedMonth), [paymentRecords, selectedMonth]);

  const recordsByStudentId = useMemo(() => monthRecords.reduce<Record<number, PaymentRecord>>((acc, record) => {
  ...
  const summary = useMemo(() => {
    return monthRecords.reduce((acc, record) => {
      ...
      acc.outstanding += Math.max(0, record.monthlyFee - record.amountPaid);
  ...
  const handleUpdate = (studentId: number, field: 'status' | 'amountPaid' | 'notes', value: string) => {
    const currentRecord = recordsByStudentId[studentId]!;

    const nextStatus: PaymentStatus = field === 'status' ? value as PaymentStatus : currentRecord.status;
    const nextAmount = field === 'amountPaid' ? Number.parseInt(value, 10) : currentRecord.amountPaid;
    ...
    <section className="content-stack payment-tracker-view">
      <div className="card payment-tracker-hero">
        <div>
          ...
          <p>Track received, partial, and pending payments for each student in one editable monthly table.</p>
        </div>
        <div className="payment-month-filter">
          <label htmlFor="payment-month">Month</label>
          <select id="payment-month" value={selectedMonth} onChange={(event) => setSelectedMonth(event.target.value)}>
      ...
      <div className="payment-summary-grid">
        ...
        <div className="card payment-summary-card status">
          ...
          <strong>{summary.paidCount} / {summary.partialCount} / {summary.pendingCount}</strong>
        </div>
      </div>
      ...
      <div className="card">
        <div className="section-header">
          <div>
            ...
            <p>Update each row to reflect the current payment status for that month.</p>
          </div>
        </div>
          ...
          <table className="snapshot-table payment-tracker-table">
            ...
            <tbody>
              {students.map((student) => {
                ...
                const balance = Math.max(0, record.monthlyFee - record.amountPaid);

                return (
                  <tr key={record.id} className={`payment-row ${getStatusClass(record.status)}`}>
                    <td>
                    ...
                    <td>
                      <TextField
                        ...
                        onChange={(event) => handleUpdate(student.id, 'amountPaid', event.target.value)}
                      />
                    </td>
                    <td>
                      <select aria-label={`${student.firstName} ${student.lastName} status`} value={record.status} onChange={(event) => handleUpdate(student.id, 'status', event.target.value)}>
                    ...
                    <td>
                      <TextField
                        ...
                        value={record.notes}
                        onChange={(event) => handleUpdate(student.id, 'notes', event.target.value)}
        ...
        <Button type="button" variant="outlined" onClick={() => setSelectedMonth(currentMonth)}>Reset to current month</Button>
      </div>
    </section>
  );
};
