...
/**
 * The bundled defaults are mirrored in the API repo, which serves them until
 * the teacher publishes (REQ-045). The mirror used to be discipline: the copy
 * drifted on 2026-08-06 and cost a debugging session, and by 2026-08-12 the
 * two had parted again over the services list and the per-session pricing
 * wording.
 *
 * This snapshot makes this repo's defaults reviewable in one plain file, and
 * the `content-drift` CI job compares it with the API repo's copy — so a
 * change to one side that never reached the other fails the build.
 *
